...
void Load(){
    ...
    if(choice==1 && !(load=fopen("save1.txt", "r") == NULL)){
        ...
        fprintf(load,"name:%s energy:%d attack:%d defense:%d money:%d xp:%d level:%d\n",&Player.name,&Player.energy,&Player.attack,&Player.defense,&Player.money,&Player.xp,&Player.level);
        fprintf(load,"coffee:%d energydrink:%d ynovCourses:%d autodebug:%d numpad:%d mouseXtraKeys:%d macroKeypad:%d",&Player.Inventory.coffee,&Player.Inventory.energyDrink,&Player.Inventory.ynovCourses,&Player.Inventory.autodebug,&Player.Inventory.numpad,&Player.Inventory.mouseXtraKeys,&Player.Inventory.macroKeypad);
        fprintf(load,"coord:%d",&coord);
        fclose(load);
